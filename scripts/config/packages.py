...
pkgDb = \
       ...
       , 'libs/graphics/graphics'              : HProject   ('flowbox-graphics'             , os.path.join ('libs' , 'graphics', 'graphics')                , 'libs'    , ['libs/target-ghchs', 'libs/utils', 'libs/num-conversion', 'libs/codec/exr', 'libs/graphics/hopencv'], flags=Flags([Flag("--with-gcc=gcc-4.9", [systems.DARWIN])]))
       , 'libs/graphics/hopencv'               : HProject   ('HOpenCV'                      , os.path.join ('libs' , 'graphics', 'hopencv')                 , 'libs'    , [])
       , 'libs/luna/base'                      : HProject   ('flowbox-luna-base'            , os.path.join ('libs' , 'luna', 'base')                        , 'libs'    , ['libs/target-ghchs', 'libs/utils', 'libs/config', 'libs/cabal-install', 'libs/markup'])
       , 'libs/luna/interpreter'               : HProject   ('flowbox-luna-interpreter'     , os.path.join ('libs' , 'luna', 'interpreter')                 , 'libs'    , ['libs/utils', 'libs/luna/base', 'libs/batch/batch'], flags=Flags([Flag('--force-reinstalls')])) # FIXME [PM] force reinstalls-flag  resolves problem with HTTP and network
       ...
       , 'libs/target-ghchs'                   : HProject   ('luna-target-ghchs'            , os.path.join ('libs' , 'target-ghchs')                        , 'libs'    , [])
