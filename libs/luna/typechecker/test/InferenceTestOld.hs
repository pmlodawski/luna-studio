...
assignLiteralTypesTest2 :: (Ref $ Node (NetLayers () :< Draft Static))
                       -> NetGraph ()
                       -> IO ((), NetGraph ())
assignLiteralTypesTest2 ref g = runNetworkBuilderT g $ assignLiteralTypes proxy ref

sampleGraph :: Show a => NetGraph a -> IO (Ref $ Node (NetLayers a :< Draft Static), NetGraph a)
sampleGraph g = runNetworkBuilderT g $ do
    i1 <- int 2
    i2 <- int 3
    i3 <- int 4
    s1 <- str "abc"
