...
type SymbolMap     l = Map QualPath (Function l)
data SymbolError     = SymbolNotFound
type SymbolBuilder l = MonadSymbolBuilder (SymbolMap l)

loadSymbols :: SymbolBuilder l m => SymbolMap l -> m ()
loadSymbols = SymbolBuilder.withSymbols_ . Map.union

lookupSymbol :: SymbolBuilder l m => QualPath -> m (Maybe (Function l))
lookupSymbol p = Map.lookup p <$> SymbolBuilder.get

-- TODO[MK,PM]: Figure out which parts of the code below we need in the future
--
