...
import           Luna.ASTNew.Decl       (Decl, RDecl, RCons, ImpTgt)
...
instance Traversal base m (RDecl f e) => DefaultTraversal base m (Module f e) where
    defaultTraverse b (Module.Module path name body) = Module.Module <$> traverse b path <*> traverse b name <*> traverse b body


----- Decl -----

instance ( Traversal base m (RDecl f e)
         , Traversal base m (RCons f e)
         , Traversal base m (RType f)
         , Traversal base m (Arg   f e)
         , Traversal base m (Native (RDecl f e))
