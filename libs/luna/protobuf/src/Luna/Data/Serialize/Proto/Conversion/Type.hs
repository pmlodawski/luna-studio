...
instance Convert Type Gen.Type where
    ...
    decode t@(Gen.Type cls mtid _) = do
        ...
        case cls of
            ...
            GenCls.Function -> do ext <- getExt GenFunction.ext
                                  (GenFunction.Function tinputs mtoutput) <- ext <?> "Failed to decode Type.Function: extension is missing"
                                  toutput <- mtoutput <?> "Failed to decode Type.Function: 'output' field is missing"
                                  Type.Function i <$> decodeList tinputs <*> decode toutput
