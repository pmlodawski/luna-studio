...
instance (a1~a2, m1~m2, mx1~mx2, Monad m1, Monad mx1, out~b) => Pipe (Value mx1 (m1 a1) -> b) (Value mx2  (m2 a2)) out where
    pipe f a = f a
...
main = do
    ...
    --print $ pure [sumInt'] >>> pure (1::Int) >>> pure (2::Int)

    putStrLn "---"

    print $ pure [sumMeSingle'] >>> pure (1::Int)
