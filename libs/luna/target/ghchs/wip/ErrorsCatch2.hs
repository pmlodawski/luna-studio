...
--instance  (Catch base e2 (UnsafeBase base e3) dstBase) =>Catch (UnsafeBase base1 e1) e1 (UnsafeBase base2 e2) (UnsafeBase dstBase e1)  where
--    catch f sa = case sa of
--        Value a -> Value a
--        Error e -> f e
--        Other o -> Other $ catch f o
...
--instance  (Catch base e2 (UnsafeBase base e3) dstBase) =>Catch (UnsafeBase base1 e1) e2 (UnsafeBase base3 e3) (UnsafeBase dstBase e1)  where
--    catch f sa = case sa of
--        Value a -> Value a
--        Error e -> Error e
--        Other o -> Other $ catch f o
