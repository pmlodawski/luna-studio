...
class TransSquash t1 t2 t3 | t1 t2 -> t3 where
    transSquash :: t1 (t2 m) a -> t3 (m a) <= Monad m


class TransJoin m1 m2 m3 | m1 m2 -> m3 where
    transJoin :: m1 (m2 a) -> m3 a

instance TransJoin Pure Pure Pure where
    transJoin (Pure a) = a

instance TransJoin Pure IO IO where
    transJoin (Pure a) = a

instance TransJoin IO Pure IO where
    transJoin a = do Pure val <- a
                     return val

instance TransJoin IO IO IO where
    transJoin a = do val <- a
                     val


instance TransJoin m m m <= (Monad m) where
    transJoin a = do val <- a
                     val

liftValF1 :: (m1 t1 -> m2 t2) -> (t1 :> m1 -> t2 :> m2)
liftValF1 f = Value . f . fromValue

flattenEnv :: v :> m2 :> m1 -> v :> mout <= (TransJoin m1 m2 mout, Functor m1)
flattenEnv = liftValF1 transJoin . fmap fromValue

transSubjoin :: t m1 (m2 a) -> t m3 a <= (TransJoin (t m1) (t m2) (t m3), Functor (t m1), MonadTrans t, Monad m2)
transSubjoin (a :: t1 m1 (m2 a)) = transJoin (fmap lift a :: t1 m1 (t1 m2 a))

--subsquash :: t m (m a) -> t m a
...
--class MatchEnv (mref :: * -> *) m mout | mref m -> mout where
--    matchEnv :: m a -> mout a


--instance MatchEnv Pure Pure Pure where
--    matchEnv = id

--instance MatchEnv Pure IO IO where
--    matchEnv = id

--instance MatchEnv IO IO IO where
--    matchEnv = id

--instance MatchEnv IO Pure IO where
--    matchEnv (Pure a) = return a

    --class MatchEnv m mout where
    --    matchEnv :: m a -> mout a


    --instance MatchEnv Pure Pure where
    --    matchEnv = id

    --instance MatchEnv IO IO where
    --    matchEnv = id

    --instance MatchEnv Pure IO where
    --    matchEnv (Pure a) = return a


class MatchEnv mref m mout | mref m -> mout where
    matchEnv :: mref ref -> m a -> mout a
...
instance MatchEnv IO Pure IO where
    matchEnv _ (Pure a) = return a


class TransFunctor t where
    tmap :: Monad m => (a -> m b) -> (t m a -> t m b)
...
newtype DataProxy a = DataProxy a deriving Show
...
instance Pipe (a1 :> m1 -> b) (DataProxy a2) b <= (a1~a2, Monad m1) where
    pipe f (DataProxy a) = f $ return a

instance Pipe (a1  :> m1   -> b)       (a2  :> m2)  (tout :> mout) <= (m1~m2, TransJoin m2 m20 mout, Monad m2, Pipe (a1 :> m1 -> b) (DataProxy a2) (tout :> m20), Functor m2) where
    pipe f ma = flattenEnv . Value $ do
        a <- fromValue ma
        return $ f >>> DataProxy a
...
instance Pipe (a1 :> m1 -> b :> mb) (IC c2 mc2 a2 :> m2) ((IC c2 m4 a) :> m2) <= (TransJoin (c2 mc2) (c2 m3) (c2 m4), Pipe (a1 :> m1 -> b :> mb) (a2 :> m1) (Value m3 a), MonadTrans c2, Functor (c2 mc2), Functor m2, Monad (c2 mc2), Monad m3, Monad m2, Monad m1, m1~Pure) where
    pipe f ca = Value . fmap (IC . transSubjoin) $ do
    ma <- fromValue ca
    return $ do 
        a <- fromIC ma
        return . fromValue $ f >>> (pureVal a)

    --xxx (f :: (a1 :> Pure -> b :> mb)) (ca :: (IC c2 mc2 a2 :> m2)) = Value . fmap (IC . transSubjoin) $ do
    --    ma <- fromValue ca
    --    return $ do 
    --        a <- fromIC ma
    --        return . fromValue $ f >>> (pureVal a)
...
--instance Pipe (a1 :> m1 -> b :> mb) (IC c2 mc2 a2 :> m2) (m2 (tout mout b)) <= (m1~mc2, a1~a2, Monad m2, Functor m2, TransJoin (c2 mc2) (c2 mb) (tout mout), MonadTrans c2, Functor (c2 mc2), Monad mb, Monad (c2 mc2), Monad mc2) where
--    pipe = undefined

--instance Pipe (IC c1 mc1 a1 :> m1 -> b) (a2  :> m2)  b  <= (m1~m2, a1~a2) where
--    pipe f a = undefined
...
--instance Pipe (a1 :> m1 -> b :> mb) (IC c2 mc2 a2 :> m2)  (IC c2 mc2 out)  <= (m1~m2, a1~a2, Pipe (a1 :> m1 -> b) (a2 :> m2) out) where
--    pipe f a = undefined
...
instance ToIO (a :> IO) (a :> IO) where
    toIO = id

instance ToIO (a :> Pure) (a :> IO) where
    toIO (fromValue -> Pure a) = return a
...
--time :: Int :>> Pure :>> StateT Int
time :: IC (StateT Int) Pure Int
time = return 5

    --nameInTime :: String :>> Pure :>> (StateT Int)
    --nameInTime = IC $ do
    --    num <- get
    --    return ("image " ++ show num)

    --readImage :: String -> String :>> Pure
    --readImage name = return ("image: " ++ name)
...
main = do
    ...
    print $ flip runStateT 0 $ fromIC (sumInt' >>> time >>> pureVal (1::Int))


    print $ flip runStateT 0 $ fromIC $ fromPure . fromValue $ (sumMeSingle' >>> pureVal time)
    
    --print $ sumMeSingle' >>> pureVal (1::Int)

    --print $ sumMeSingle' >>> pureVal [ pureVal (1::Int) ]

    --print =<< (fromValue . toIO) (sumInt' >>> pureVal (1::Int) >>> pureVal (1::Int))
    --print $ sumMeSingle'   >>> pureVal (1::Int)
    --print =<< (toIO $ runStateT (fromIC $ readImage' >>> nameInTime) 10)
