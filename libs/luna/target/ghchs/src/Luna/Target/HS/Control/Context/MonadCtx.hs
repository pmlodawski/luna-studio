...
--instance AppMonadCtx (Value m2 s2 a2) (Req req (MonadCtx2 env set m1 s1) a1) <= (s2~s1, a1~Value Pure Safe a2, Functor s1, Functor m2, LiftValue m2 m1) where
--    appMonadCtx = Req . MonadCtx2 . liftValue . (fmap (Value . Pure . Safe))
...
instance AppMonadCtx (Value m2 s2 a2) (Req req (MonadCtx2 env set m1 s1) a1) <= (set~Insert req Empty, s1~s2, m1~t (Value m2), a1 ~ Value Pure Safe a2, LiftValue' (Value m2) s2 t, Functor s2, Functor m2) where
    appMonadCtx = Req . MonadCtx2 . liftValue' . (fmap (Value . Pure . Safe))
...
--liftValue :: m s a -> t m s a
--liftValue :: Value m s a -> t m s a <- tak powinna wygladac ta sygnatura!!!
