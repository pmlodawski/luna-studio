...
generateSel n = do 
    ...
    putStrLn "module Flowbox'.Data.NTuple.Select where"
...
generateSelNinst j i = do
    putStrLn $ "instance C''select" ++ show j ++ " (" ++ intercalate ",(" ["a" ++ show l | l <- [0..i]] ++ ",()" ++ replicate (i+1) ')' ++ " a" ++
               show j ++ " where select" ++ show j ++ " (" ++ 
               intercalate ",(" [if l == j then "x,_" else "_" | l <- [0..j]] ++ replicate (j+1) ')' ++" = x"
               ++ "; "
