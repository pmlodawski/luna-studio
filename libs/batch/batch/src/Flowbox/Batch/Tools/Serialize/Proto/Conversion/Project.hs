...
instance Convert (Project.ID, Project) (Gen.Project, LibManager, ProcessMap) where
    encode (projectID, project) = (tproject, libs, processMap) where
        Project name path libPaths libs attrs processMap = project
        tname      = encodePJ name
        tpath      = encodePJ path
    ...
    decode (Gen.Project mtname mtpath tlibPaths mtattrs mtprojectID, libs, processMap) = do
        ...
        let name      = decodeP tname
            ...
            project   = Project name path libPaths libs attrs processMap
