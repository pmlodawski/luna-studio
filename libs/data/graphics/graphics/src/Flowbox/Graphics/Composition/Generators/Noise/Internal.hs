...
gradientNoise3D fx fy fz ix iy iz seed = ((xvGrad * xvPoint) + (yvGrad * yvPoint) + (zvGrad * zvPoint)) * 2.12
  where
    ...
    vectorIndex'' = (xNoiseGen * ix + yNoiseGen * iy + zNoiseGen * iz + seedNoiseGen * seed) .&. 0xffffffff
    vectorIndex' = vectorIndex'' `xor` (vectorIndex'' `A.shiftR` shiftNoiseGen)
...
gradientCoherentNoise quality seed x y z = linear iy0 iy1 zs
  where
    ...
    zs = scurveMapFun (z - A.fromIntegral z0)
