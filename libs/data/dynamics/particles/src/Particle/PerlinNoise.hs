...
makeLenses ''PerlinNoise

instance Default PerlinNoise where
  def = PerlinNoise 1.0 2.0 6 0.5 0 63 112 0.0 1.0
-- FIXME [MM]: default values taken straight from Math.Noise.Modules.{Perlin,ScaleBias}
--             sources, except from seeds - those are made up

-- FIXME [MM]: runPerlinNoise is impossible to construct with pipelined architecture and pure
--             Haskell Noise implementation
--             Calling run/run1 in pipeline causes Accelerate error: could not construct cyclic
--             Acc type
--runPerlinNoise :: PerlinNoise -> Backend -> PipelinedSystem
--runPerlinNoise params backend ps = A.lift (timediff, pos, vel, newAccelerations, age)
--  where
--    (timediff, pos, vel, acc, age) = A.unlift ps :: UnliftedPipelinedSystem
--    newAccelerations = acc .+ A.use (noiseVectors ^. listArray)

--    noiseVectors = noise3DArray (mkPerlin params) (A.toList $ backend pos)
...
noise3DArray :: (Noise.NoiseModule, Noise.NoiseModule, Noise.NoiseModule) -> [Position] ->
                [(Float, Float, Float)]
noise3DArray (pX, pY, pZ) pos = zip3 (noiseArray pX pos) (noiseArray pY pos) (noiseArray pZ pos)

noiseArray :: Noise.NoiseModule -> [Position] -> [Float]
noiseArray perlin = map (getValue' perlin)

getValue' :: Noise.NoiseModule -> Position -> Float
getValue' perlin pos = Float.double2Float result
  where
    Just result = Noise.getValue perlin $ over each Float.float2Double pos

mkPerlin :: PerlinNoise -> (Noise.NoiseModule, Noise.NoiseModule, Noise.NoiseModule)
mkPerlin p = (perlinX `Noise.isSourceOf` scaleBias, perlinY `Noise.isSourceOf` scaleBias, perlinZ `Noise.isSourceOf` scaleBias)
  where
    perlinX   = Noise.Perlin (p ^. frequency) (p ^. lacunarity) (p ^. octaves) (p ^. persistence) (p ^. seedX)
    perlinY   = Noise.Perlin (p ^. frequency) (p ^. lacunarity) (p ^. octaves) (p ^. persistence) (p ^. seedY)
    perlinZ   = Noise.Perlin (p ^. frequency) (p ^. lacunarity) (p ^. octaves) (p ^. persistence) (p ^. seedZ)
    scaleBias = Noise.ScaleBias (p ^. bias) (p ^. scale)


-- pure accelerated perlin noise

-- FIXME[MM]: not tested, translated straight from Noise sources to Accelerate

positionToPerlinNoise :: A.Exp Float -> A.Exp Float -> A.Exp Float -> A.Exp Float
positionToPerlinNoise x y z = perlinGen x y z 1.0 2.0 6 0.5 123

positionsToPerlins :: A.Acc (Array Position) -> A.Acc (Array Float)
positionsToPerlins = A.map f
  where
    f :: A.Exp Position -> A.Exp Float
    f args = positionToPerlinNoise x y z
      where
        (x,y,z) = A.unlift args :: (A.Exp Float, A.Exp Float, A.Exp Float)
...
gradientNoise3D fx fy fz ix iy iz seed = ((xvGrad * xvPoint) + (yvGrad * yvPoint) + (zvGrad * zvPoint) * 2.12)
  where
    ...
    yvPoint = fy - A.fromIntegral iy
    zvPoint = fz - A.fromIntegral iz

    xvGrad = staticTable A.!! index
    yvGrad = staticTable A.!! index + 1
    zvGrad = staticTable A.!! index + 2

    clamped = t A.>* 255 A.? (255, t)
      where t = icalc `Bits.xor` (icalc `A.shiftR` shiftNoiseGen)
    index = clamped `A.shiftL` 2
    icalc = xNoiseGen * ix + yNoiseGen * iy + zNoiseGen * iz + seedNoiseGen * seed

gradientCoherentNoise :: A.Exp Float -> A.Exp Float -> A.Exp Float -> A.Exp Int -> A.Exp Float
gradientCoherentNoise x y z seed = linear iy0 iy1 zs
  where
    x0 :: A.Exp Int
    x0 = A.truncate (x A.>* 0.0 A.? (x, x - 1))
    x1 = x0 + 1
    y0 :: A.Exp Int
    y0 = A.truncate (y A.>* 0.0 A.? (y, y - 1))
    y1 = y0 + 1
    z0 :: A.Exp Int
    z0 = A.truncate (z A.>* 0.0 A.? (z, z - 1))
    z1 = z0 + 1

    xs = scurve3 (x - A.fromIntegral x0)
    ys = scurve3 (y - A.fromIntegral y0)
    zs = scurve3 (z - A.fromIntegral z0)

    n0a = gradientNoise3D x y z x0 y0 z0 seed
    n1a = gradientNoise3D x y z x1 y0 z0 seed
    ix0a = linear n0a n1a xs

    n0b = gradientNoise3D x y z x0 y1 z0 seed
    n1b = gradientNoise3D x y z x1 y1 z0 seed
    ix1a = linear n0b n1b xs
    iy0 = linear ix0a ix1a ys

    n0c = gradientNoise3D x y z x0 y0 z1 seed
    n1c = gradientNoise3D x y z x1 y0 z1 seed
    ix0b = linear n0c n1c xs

    n0d = gradientNoise3D x y z x0 y1 z1 seed
    n1d = gradientNoise3D x y z x1 y1 z1 seed
    ix1b = linear n0d n1d xs
    iy1 = linear ix0b ix1b ys

perlinGen :: A.Exp Float -> A.Exp Float -> A.Exp Float ->
             A.Exp Float -> A.Exp Float ->
             A.Exp Int -> A.Exp Float -> A.Exp Int -> A.Exp Float
perlinGen x y z freq lac octaveCount persistence seed =
  value $ A.iterate (octaveCount - 1) octaveFunc (A.lift ((0.0 :: Float), (1.0 :: Float), x*freq, y*freq, z*freq, (1 :: Int)))
  where
    value args = val
      where
        (val, curPersistence, ox, oy, oz, curOctave) = A.unlift args :: (A.Exp Float, A.Exp Float, A.Exp Float, A.Exp Float, A.Exp Float, A.Exp Int)

    signal sx sy sz octv = gradientCoherentNoise sx sy sz (seed + octv)

    octaveFunc args =
      A.lift (val + (signal ox oy oz (curOctave + 1) * curPersistence)
...
xNoiseGen, yNoiseGen, zNoiseGen, seedNoiseGen, shiftNoiseGen :: A.Exp Int
xNoiseGen = 1619
yNoiseGen = 31337
zNoiseGen = 6971
seedNoiseGen = 1013
