...
pLambda i     = Lambda     <$> (pTuplePure (pOpExpr2 i) <<|> liftList pIdent')
                           <*> pExprBlock i -- <* pSpaces <* pMany pSegmentEmpty--pSpaces is used because of micro usage
                           --`micro` 1
...
pOp       sym = Operator   <$> (pSyms sym)
pAssignOp sym = Assignment <$  (pSyms sym)
pAccOp    sym = Accessor   <$  (pSyms sym)
...
parse p s = UU.parse ( (,) <$> amb(p) <*> pEnd) (createStr (LineColPos 0 0 0) (s ++ ""))
