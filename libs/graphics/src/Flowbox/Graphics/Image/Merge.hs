...
-- TODO: should there be a third Channel.Select for outgoing channels? same thing can be achieved by not selecting
--       channels from the image we're merging with
merge :: (A.Shape ix, A.Elt a, A.IsFloating a) => ImageAcc ix a -> ImageAcc ix a -> MergeOperation ix a -> Channel.Select -> Channel.Select
    -> Maybe (Mask ix a) -> Exp a
    -> Image.Result (ImageAcc ix a)
merge overlay background mergeType channelsOv channelsBg mask mix = do
    -- use mix ...
    result <- applyMerge mergeType overlay' background'
    Comp.maskWith result background mask
