...
instance (MonadIO m, Show a) => LogHandler PrintHandler m a where
    handle _ = liftIO . print

data Handler2 = Handler2

data LogState s = LogState { _msgs     :: Seq (Message s) 
                             --, _handlers :: forall h. LogHandler h m s => [h]
                             , _lvl      :: Int
                             }


instance Monoid (LogState s) where
    mempty = LogState mempty 0
    --(LogState m f) `mappend` (LogState m' f') = LogState (m `mappend` m')
    --                                                     (f `mappend` f')

makeLenses ''LogState

newtype LoggerT s m a = LoggerT { unLoggerT :: StateT (LogState s) m a } deriving (Monad, MonadIO, Applicative, Functor)
type    Logger s a = LoggerT s Identity a

--data Logger t = Logger deriving (Show)

data HandledT m a = HandledT { runHandledT :: m a } deriving (Show)    

instance Monad m => Monad (HandledT m) where
    return = HandledT . return
    (HandledT ma) >>= f = HandledT $ do
        a <- ma
        runHandledT $ f a

instance Functor m => Functor (HandledT m) where
    fmap f (HandledT a) = HandledT $ fmap f a

instance MonadIO m => MonadIO (HandledT m) where
    liftIO = HandledT . liftIO

instance (Monad m, Functor m, Applicative m) => Applicative (HandledT m) where
    pure = return
    (HandledT mf) <*> (HandledT ma) = HandledT $ mf <*> ma
...
instance Monad m => MonadLogger s (LoggerT s m) where
    logMsg msg = withLogState (msgs %~ (<> return msg))

instance (MonadLogger s a, MonadIO a, Show s) => MonadLogger s (HandledT a) where
    logMsg msg = liftIO $ print msg
...
main = do
    print =<< (view msgs . snd <$> runLoggerT ( runHandledT test :: StdLogger IO ())   )

    --runLogger $ withHandler PrintHandler test
