...
data Pri = Pri deriving (Show)
type instance DataOf Pri = Int
...
instance (MonadIO m, Applicative m, f~msg) => LogHandler (PrintHandler f) m msg where
    handle h msg = do
        ...
        liftIO $ putStrLn $ runFormatter formatter msg
        runChildren h msg
...
data Formatter a = Formatter { runFormatter :: Log a -> String }
...
(<:>) :: (FormatterBuilder a (Formatter c), FormatterBuilder b (Formatter c)) => a -> b -> Formatter c
...
instance FormatterBuilder String (Formatter a) where
    buildFormatter a = Formatter $ const a

instance Lookup Msg a => FormatterBuilder Msg (Formatter a) where
    buildFormatter a = Formatter $ pprint . readData a

instance (a~b) => FormatterBuilder (Formatter a) (Formatter b) where
    buildFormatter = id
...
type StdLogger m a = LoggerT (DataRecord Msg,()) m a
