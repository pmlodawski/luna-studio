...
class TransSquash t1 t2 t3 | t1 t2 -> t3 where
    transSquash ::  Monad m=>t1 (t2 m) a -> t3 (m a)  
...
instance  (Monad m) =>TransJoin m m m  where
    transJoin a = do val <- a
                     val
...
flattenEnv ::  (TransJoin m1 m2 mout, Functor m1)=>v :> m2 :> m1 -> v :> mout  
flattenEnv = liftValF1 transJoin . fmap fromValue

transSubjoin ::  (TransJoin (t m1) (t m2) (t m3), Functor (t m1), MonadTrans t, Monad m2)=>t m1 (m2 a) -> t m3 a  
...
instance  (a1~a2, Monad m1) =>Pipe (a1 :> m1 -> b) (DataProxy a2) b  where
    pipe f (DataProxy a) = f $ return a

instance  (m1~m2, TransJoin m2 m20 mout, Monad m2, Pipe (a1 :> m1 -> b) (DataProxy a2) (tout :> m20), Functor m2) =>Pipe (a1  :> m1   -> b)       (a2  :> m2)  (tout :> mout)  where
...
instance  (TransJoin (c2 mc2) (c2 m3) (c2 m4), Pipe (a1 :> m1 -> b :> mb) (a2 :> m1) (Value m3 a), MonadTrans c2, Functor (c2 mc2), Functor m2, Monad (c2 mc2), Monad m3, Monad m2, Monad m1, m1~Pure) =>Pipe (a1 :> m1 -> b :> mb) (IC c2 mc2 a2 :> m2) ((IC c2 m4 a) :> m2)  where
...
--instance  (m1~mc2, a1~a2, Monad m2, Functor m2, TransJoin (c2 mc2) (c2 mb) (tout mout), MonadTrans c2, Functor (c2 mc2), Monad mb, Monad (c2 mc2), Monad mc2) =>Pipe (a1 :> m1 -> b :> mb) (IC c2 mc2 a2 :> m2) (m2 (tout mout b))  where
--    pipe = undefined

--instance  (m1~m2, a1~a2) =>Pipe (IC c1 mc1 a1 :> m1 -> b) (a2  :> m2)  b   where
--    pipe f a = undefined
...
--instance  (m1~m2, a1~a2, Pipe (a1 :> m1 -> b) (a2 :> m2) out) =>Pipe (a1 :> m1 -> b :> mb) (IC c2 mc2 a2 :> m2)  (IC c2 mc2 out)   where
--    pipe f a = undefined
