    ...
    , runPreProcessor = mkSimplePreProcessor $ \inFile outFile verbosity -> do
        ...
        let moduleName = concat $ List.intersperse "." $ tail $ FilePath.splitDirectories $ FilePath.dropExtension inFile
            ...
            contents = unlines [
                ...
                , "import qualified Flowbox.Graphics.Color.Companding as C (fromLinear, toLinear)"
                ...
                , ""
                , "    fromLinear c@(" ++ name ++ " rgb) = " ++ name ++ " $ rgb & each %~ (C.fromLinear (gamma c))"
                , ""
                , "    toLinear   c@(" ++ name ++ " rgb) = " ++ name ++ " $ rgb & each %~ (C.toLinear   (gamma c))"
                , ""
                , "    gamma _ = " ++ gamma
                , ""
                ]
