...
toAction (Mouse event@(Mouse.Event Mouse.Released _   Mouse.LeftButton _ _)) = Just $ stopDrag event
toAction _                                                                   = Nothing
...
handleMove coord = do
    ...
    forM_ connectingMay $ \connecting@(Connecting sourceRef sourceWidget sourceVector nodePos _ (DragHistory start current)) -> do
        graph  <- use Global.graph
        camera <- use $ Global.camera . Camera.camera

        Global.connect . Connect.connecting %= (fmap $ Connect.history . Connect.dragCurrentPos .~ coord)
...
stopDrag :: Mouse.Event -> Command State ()
stopDrag event@(Mouse.Event _ coord _ _ mayEvWd) = do
    connectingMay <- use $ Global.connect . Connect.connecting
    forM_ connectingMay $ \connecting@(Connecting sourceRef _ _ _ _ (DragHistory start current)) -> do
        graph <- use Global.graph
