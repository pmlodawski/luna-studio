...
--     lookup oid    = (getFromRegistryJS oid >>= return . wrapWidget . JSObject.fromJSVal)
--
...
--     bref     <- getFromRegistryJS oid >>= return . JSObject.fromJSVal
--     uniforms <- JSObject.getProp "uniforms"                          bref >>= return .           JSObject.fromJSVal
--     uniform  <- JSObject.getProp (JSString.pack $ uniformName n)  uniforms >>= return . Uniform . JSObject.fromJSVal
--     setValue uniform v
