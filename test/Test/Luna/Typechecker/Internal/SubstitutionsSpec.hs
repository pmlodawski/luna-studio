...
module Test.Luna.Typechecker.Internal.SubstitutionsSpec (spec) where

import qualified Luna.Typechecker.Internal.AST.Kind         as Knd
import qualified Luna.Typechecker.Internal.AST.Type         as Ty
import           Luna.Typechecker.Internal.AST.Type         (Type(TVar), fn)
...
spec = do
  ...
  describe "Symmetric substitutions composition (merge)" $ do
    it "satisfies property: apply (s1++s2) = apply (s2++s1)" $
      property $
        forAll arbitrary         $ \(t :: Type)  -> 
        forAll (genSubst (tv t)) $ \s1 ->
        forAll (genSubst (tv t)) $ \s2 ->
          let s12 = merge s1 s2
           in isJust s12 ==> apply (fromJust s12) t == (apply s1 . apply s2) t
