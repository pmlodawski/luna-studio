...
runThreadedLogger f m = do
    ...
    return out
    where loop :: (MonadIO m, Applicative m) => MVar () -> (OutChan (ChMsg m)) -> m ()
          loop sync ch = do
...
instance (MonadIO m, LogConstructor d n) => LogConstructor d (ThreadedLogger n m) where
...
instance (MonadIO m, MonadPriorityLogger d) => MonadPriorityLogger (ThreadedLogger d m) where
...
--runThreadedLogger2 :: (MonadIO m, MonadIO n, Applicative n) => (n () -> IO ()) -> ThreadedLogger n m a -> m a
--runThreadedLogger2 :: (MonadIO m, Applicative m) => ThreadedLogger m (LoggerT (Data Lvl, (Data Msg, ())) IO) () -> m ()
runThreadedLogger2 :: (MonadIO m, Applicative m, l~LogFormat m) => ThreadedLogger m (LoggerT l IO) () -> m ()
runThreadedLogger2 m = do
    sync              <- liftIO newEmptyMVar
    (inChan, outChan) <- liftIO newChan
    liftIO $ forkIO $ do
        out <- runLoggerT' $ flip runReaderT inChan . fromThreadedLogger $ m
        writeChan inChan End
        return out
    loop sync outChan
    where loop :: (MonadIO m, Applicative m) => MVar () -> (OutChan (ChMsg m)) -> m ()
          loop sync ch = do
...
newtype LoggerT l m a = LoggerT { fromLoggerT :: m a } deriving (Monad, MonadIO, Applicative, Functor) 
...
runLoggerT :: (Functor m, Monad m) => l -> LoggerT (MapRTuple Data (Tuple2RTuple l)) m a -> m a
runLoggerT _ = fromLoggerT

runLoggerT' = fromLoggerT
...
main = do
    --print =<< (runLoggerT (Lvl, Msg) $ runHandlerLoggerT $ runPriorityLoggerT Warning test)
    --print =<< runThreadedLogger (runLoggerT (Lvl,Msg) . runHandlerLoggerT . runPriorityLoggerT Warning) test

    --print $ runIdentity (runLoggerT (Lvl, Msg) test)
    print =<< (runLoggerT (Lvl, Msg) . runHandlerLoggerT . runPriorityLoggerT Warning . runThreadedLogger2 $ test)
