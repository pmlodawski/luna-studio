...
runLoggerT2' :: (Functor m, Monad m) => l -> LoggerT2 (MapRTuple Data (Tuple2RTuple l)) m b -> m b
runLoggerT2' _ = runLoggerT2
...
instance (Functor m, Monad m, LogBuilder d (LoggerT2 l m) l) => LogConstructor d (LoggerT2 l m) where
    mkLog = defMkLog
...
data ChMsg d = ChMsg (RecordBuilder d) | End

--runThreadedLogger :: MonadIO m => ThreadedLogger m a -> m a
--runThreadedLogger :: MonadIO m => f -> ThreadedLogger d m a -> m a
runThreadedLogger f m = do
    (inChan, outChan) <- liftIO newChan
    out <- flip runReaderT inChan . fromThreadedLogger $ m
    liftIO $ forkIO $ f $ loop outChan
    return out

loop :: (MonadIO m, MonadLogger m, LogBuilder d m (LogFormat m)) => (OutChan (ChMsg d)) -> m ()
loop ch = do
    l <- liftIO $ readChan ch
    case l of
        End     -> return ()
        ChMsg d -> defMkLog d *> loop ch

--type instance LogFormat (ThreadedLogger l m) = l

getChan = ThreadedLogger Reader.ask

instance (d~d', MonadIO m) => LogConstructor d (ThreadedLogger d' m) where
    mkLog d = do
        ch <- getChan
        liftIO $ writeChan ch (ChMsg d)
...
main = do
    ...
    --print =<< runLoggerT2' (Lvl, Msg) test
    --print =<< (runLoggerT2 ( test :: StdLogger2 IO ()   ))
    print =<<  runThreadedLogger (runLoggerT2' (Lvl, Msg)) test
