...
runThreadedLogger :: (MonadIO m, Applicative m) => ThreadedLogger m (LoggerT l IO) a -> m a
runThreadedLogger m = do
    ...
    where loop :: (MonadIO m, Applicative m) => (OutChan (ChMsg m)) -> m ()
          loop ch = do
...
main = do
    ...
    --print =<< (runLoggerT (Lvl, Msg) . runThreadedLogger2 $ test2)

    print =<< (runLoggerT (Lvl, Msg) . runHandlerLoggerT . runPriorityLoggerT Warning . runThreadedLogger $ test)
