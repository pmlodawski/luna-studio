...
instance (PPrint (DataOf seg), Lookup seg a) => FormatterBuilder seg a where
    buildFormatter a = Formatter $ pprint . readData a
...
instance (LogBuilder' d (LoggerT log m))
         => LogConstructor d (LoggerT log m) where
    mkLog d = do
        log <- buildLog' d
        appendLog log -- $ fmap r2t log
...
newtype PriorityLoggerT log m a = PriorityLoggerT { unPriorityLoggerT :: StateT Int m a } deriving (Monad, MonadIO, Applicative, Functor, MonadTrans)

--instance Monad m => MonadLogger log (PriorityLogger log m) where
--    appendLog log = do
--        s <- get2
--        put2 (s |> log)
--    --flush = do
...
runPriorityLoggerT pri = fmap fst . flip runStateT pri . unPriorityLoggerT
...
main = do
    print =<< (runLoggerT ( (runPriorityLoggerT 0 test) :: StdLogger IO () ))
