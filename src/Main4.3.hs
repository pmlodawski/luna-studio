...
class MapRTupleRec a b t t' | t -> a, t b -> t' where
    mapRTupleRec :: (a -> b) -> t -> t'

instance (MapRTupleRec a b as bs) => MapRTupleRec a b (a,as) (b,bs) where
    mapRTupleRec f (a,b) = (f a, mapRTupleRec f b)

type family IsEmpty a where
    IsEmpty ()     = True
    IsEmpty (a,as) = False

class MapRTuple f t t' where
    mapRTuple :: f -> t -> t'

instance (empty~IsEmpty t, MapRTuple' empty f t t') => MapRTuple f t t' where
    mapRTuple = mapRTuple' (Proxy :: Proxy empty)

class MapRTuple' (empty :: Bool) f t t' where
    mapRTuple' :: Proxy empty -> f -> t -> t'

instance (t~t') => MapRTuple' True f t t' where
    mapRTuple' _ _ = id

instance (t~(a->b), t'~(j -> k), MapRTupleRec a b j k)
      => MapRTuple' False f t t' where
    mapRTuple' _ _ = mapRTupleRec
...
newtype Func      (names :: [Symbol]) defs f   = Func { fromFunc :: (f,defs) } deriving (Functor)
newtype FuncTrans (names :: [Symbol]) defs f a = FuncTrans { runFuncTrans :: f -> Func names defs a }

remapFunc :: Func n d f -> Func n' d f
remapFunc = Func . fromFunc

runFunc2 (Func (f, defs)) = appRTuple2 defs f
--runFunc3 (Func (f, defs)) =  (mapRTuple fromProvided defs) f
...
appByName (_::Proxy n) a (Func (f,d) :: Func ns d f) = 
    Func $ ( appNth (Proxy :: Proxy (ElemIndex n ns)) a f
           , deleteNth (Proxy :: Proxy (ElemIndex n ns)) d
           )
...
newtype Arg (name :: Symbol) a = Arg a
...
sig = (arg::Arg "x" Unprovided,(arg::Arg "y" Unprovided,(arg::Arg "z" Unprovided,())))

main = do
    let f = Func (tst, (Unprovided,(Unprovided,(Unprovided,())))) :: Func ["x","y","z"] (Unprovided,(Unprovided,(Unprovided,()))) (a->b->c->(a,b,c))
        g = appArg 7 $ appArg 5 $ appByName (Proxy::Proxy "y") [] f

    print $ runFunc2 g

    let f = func (Unprovided,(Unprovided,(Provided 5,()))) :: Num a => FuncTrans ["x","y","z"] (Unprovided,(Unprovided,(Provided a,()))) f f
