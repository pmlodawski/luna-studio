...
instance (FromList a, Default l) => FromList (HResizable l a) where fromList = HResizable def . fromList

instance HasDataStore a => HasDataStore (HResizable l a) where dataStore = layered . dataStore
instance (IsDataStore a, Default l) => IsDataStore (HResizable l a) where fromDataStore = HResizable def . fromDataStore


type instance Unlayered (HResizable s a) = a
instance Layered (HResizable s a) where
    layered = lens (\(HResizable s a) -> a) (\(HResizable s _) a -> HResizable s a)
