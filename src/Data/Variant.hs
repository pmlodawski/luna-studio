...
type CheckConsByVariant variant m cons = MaybeConsByIdx (variant `Index` (Variants cons)) variant m cons
...
wrappedCons :: forall variant m cons. CheckConsByVariant variant m cons
            => variant -> m cons
wrappedCons = maybeConsByIdx (Proxy :: Proxy (variant `Index` (Variants cons)))
