...
newtype (a ~ ToSet a) => Record a = Record (RecordTemplate (ToSet a)) deriving (Typeable)
...
instance (a ~ ToSet a, VariantShow (RecordTemplate a)) => Show (Record a) where
...
type family VariantReprsOf (a :: k) :: [[Nat]]

type instance VariantReprsOf (Record vars) = VariantReprsOf vars
...
instance (ConsByIdx (Just n) a (RecordTemplate r), ToSet r ~ r) => ConsByIdx (Just n) a (Record r) where
...
instance (Cast (RecordTemplate vars) r, ToSet vars ~ vars) => Cast (Record vars) r where
...
instance (ToSet vars ~ vars, RecordProc tp (RecordTemplate vars) out) => RecordProc tp (Record vars) out where
    recordProc tp (Record a) = recordProc tp a

instance (ToSet vars ~ vars, RecordPolyProc tp (RecordTemplate vars) out) => RecordPolyProc tp (Record vars) out where
