...
class VariantPolyProcM tp a b where
    variantPolyProcM :: tp -> a -> b
...
newtype Path (segs :: [*]) = Path [Segment segs]
...
instance SegmentExpander segs Up where
    expandSegment segs _ = tail segs
...
type SegmentExpandCtx p p' = ( IsSet p
                             , RecordPolyProc (VariantSegmentExpand (Segment p') (Diff p' p)) (RecordTemplate p) [Segment p']
                             )
data VariantSegmentExpand r (segs :: [*]) = VariantSegmentExpand { unXXX :: [r] }

instance (selected ~ In a segs, SegmentCondExpander selected a (Segment r))
      => VariantPolyProc (VariantSegmentExpand (Segment r) segs) a [Segment r] where
    variantPolyProc (VariantSegmentExpand ss) = expandSegmentCond (Proxy :: Proxy selected) ss

class SegmentCondExpander (cond :: Bool) a r where
    expandSegmentCond :: Proxy cond -> [r] -> a -> [r]

instance VariantCons (Record r) a => SegmentCondExpander False a (Segment r) where
    expandSegmentCond _ ss s = ss <> [Segment $ cons s]

instance (SegmentExpander r a) => SegmentCondExpander True a (Segment r) where
    expandSegmentCond _ = expandSegment
    --expandSegmentCond _ ss s = ss <> [Segment $ cons s]


class SegmentExpander segs s where
    expandSegment :: [Segment segs] -> s -> [Segment segs]
...
expandPathSegment :: forall segDiff t segs.
                 (RecordPolyProc (VariantSegmentExpand (Segment segs) segDiff) (RecordTemplate t) [Segment segs], IsSet t)
              => Proxy segDiff -> [Segment segs] -> Segment t -> [Segment segs]
expandPathSegment segSelection expSegs = \case
    Root      -> expSegs <> [Root]
    Node    s -> expSegs <> [Node s]
    Segment s -> recordPolyProc (vse expSegs) s
    where vse a = VariantSegmentExpand a :: VariantSegmentExpand (Segment segs) segDiff


--expandPathSegment2 :: forall segDiff t segs.
--                 (RecordPolyProc (VariantSegmentExpand (Record segs) segDiff) (RecordTemplate t) [Record segs], IsSet t)
--              => Proxy segDiff -> [Record segs] -> Record t -> [Record segs]
--expandPathSegment2 segSelection ss s = recordPolyProc (vse ss) s
--    where vse a = VariantSegmentExpand a :: VariantSegmentExpand (Record segs) segDiff

--ROZDZIELIC ^^^ NA DWIE FUNKCJE ?
--ZASTOSOWAC vvv FOLDL I W VariantSegmentExpand PRZEKAZYWAC ROZWINIETY PATH
--expandPathSegment POWINNA ZWRACAC SEGMENT z recordPolyProc (!) BO MOZNA EXPANDOWAC DO SEGMENTOW!

expandPath :: forall p p'. (SegmentExpandCtx p p') => Path p -> Path p'
expandPath (Path segs) = Path $ foldl (expandPathSegment (Proxy :: Proxy (Diff p' p))) [] segs

expandUp :: (p' ~ Remove Up p, SegmentExpandCtx p p') => Path p -> Path p'
expandUp = expandPath

expandVar :: (p' ~ Remove Var p, SegmentExpandCtx p p') => Path p -> Path p'
