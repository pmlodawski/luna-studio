...
-- see the implementation of 'decodeFileOrFail' in "Generator.Binary".
-----------------------------------------------------------------------------
...
import Generator.Binary.Get.Internal hiding ( Decoder(..), runGetIncremental )
import qualified Generator.Binary.Get.Internal as I
...
runGetState g lbs0 pos' = go (runGetIncremental g) lbs0
  ...
  go (Fail _ pos msg) _ =
    error ("Generator.Binary.Get.runGetState at position " ++ show pos ++ ": " ++ msg)
...
runGet g lbs0 = feedAll (runGetIncremental g) lbs0
  ...
  feedAll (Fail _ pos msg) _ =
    error ("Generator.Binary.Get.runGet at position " ++ show pos ++ ": " ++ msg)
