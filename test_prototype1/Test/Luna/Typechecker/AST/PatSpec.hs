...
spec = do
  describe "tiPat" $ do
    it "just works for PVar" $ do
      let v = PVar (VarID "lel")
          Right (ps, as, TVar t) = startTI $ evalLoggerT $ tiPat v
      ps `shouldBe` []
      as `shouldContain` [VarID "lel" :>: toScheme (TVar t)]
    ...
    it "just works for PAs+PVar" $ do
      let v = PAs (VarID "lel") (PVar (VarID "lol"))
          Right (ps, as, TVar t) = startTI $ evalLoggerT $  tiPat v
      ps `shouldBe` []
      as `shouldContain` [VarID "lel" :>: toScheme (TVar t)]
      as `shouldContain` [VarID "lol" :>: toScheme (TVar t)]
      evalLogger (kind t) `shouldBe` Right Star

    it "just works for PAs+PCon" $ do
      let v = PAs (VarID "lel") (PCon (VarID "(:)" :>:cons_type) [PVar (VarID "x"), PVar (VarID "xs")])
          ...
          Right (Forall [] ([] :=> x'))   = evalLogger $ find (VarID "x")   as
          Right (Forall [] ([] :=> xs'))  = evalLogger $ find (VarID "xs")  as
          Right (Forall [] ([] :=> lel')) = evalLogger $ find (VarID "lel") as
          x = apply s x'
          xs = apply s xs'
          lel = apply s lel'
      ps `shouldBe` []
    ...
    it "just works for PCon" $ do
      let v = PCon (VarID "(:)" :>:cons_type) [PVar (VarID "x"), PVar (VarID "xs")]
          cons_type = Forall [Star] ([] :=> (TGen 0 `fn` list (TGen 0) `fn` list (TGen 0)))
          Right (ps, as, TVar t) = startTI $ evalLoggerT $ tiPat v
          Right (Forall [] ([] :=> x))  = evalLogger $ find (VarID "x")  as
          Right (Forall [] ([] :=> xs)) = evalLogger $ find (VarID "xs") as
  ...
  describe "(coverage booster)" $
    it "show" $ do
      let v = PCon (VarID "(:)" :>:cons_type) [PVar (VarID "x"), PVar (VarID "xs")]
          cons_type = Forall [Star] ([] :=> (TGen 0 `fn` list (TGen 0) `fn` list (TGen 0)))
      length (concatMap show [PVar (VarID "a"), PWildcard, PAs (VarID "a") (PVar (VarID "b")), PLit (LitInt 123), v]) `shouldSatisfy` (>0)
